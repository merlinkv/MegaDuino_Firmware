    ...
    if(start==1 && (oldMotorState!=motorState) && mselectMask==1 ) {  
      if(motorState==1 && pauseOn==0){
        ...
        lcd.setCursor(0,1); lcd.print("------ PAUSED ------");    
          ...
          setXY(0,2); sendStr((unsigned char *)"---- PAUSED ----");
        #endif
        #ifdef XY2
      ...
      if(motorState==0 && pauseOn==1) {
          ...
          lcd.setCursor(13,1); lcd.print("000");
          if (lcdsegs % 100 != 0){itoa(lcdsegs%100,PlayBytes,10);lcd.setCursor(14,1);lcd.print(PlayBytes);} // // recálculo de decenas ... 10,20,30,40,50,60,70,80,90,110,120,..
          else 
          if (lcdsegs % 1000 != 0) {itoa(lcdsegs%1000,PlayBytes,10);lcd.setCursor(13,1);lcd.print(PlayBytes);} // recálculo de centenas ... 100,200,300,400,500,600,700,800,900,1100, ...
          lcdsegs++;                
        #elif defined(LCD20)
          String filename2 = fileName;
          lcd.setCursor(0,0); lcd.print("                    ");
          ...
          if (lcdsegs % 100 != 0){itoa(lcdsegs%100,PlayBytes,10);lcd.setCursor(18,1);lcd.print(PlayBytes);} // recálculo de decenas ... 10,20,30,40,50,60,70,80,90,110,120,..
          else 
          if (lcdsegs % 1000 != 0) {itoa(lcdsegs%1000,PlayBytes,10);lcd.setCursor(17,1);lcd.print(PlayBytes);} // recálculo de centenas ... 100,200,300,400,500,600,700,800,900,1100, ...
          lcdsegs++;
        #elif defined(OLED1306)
          #ifdef XY
            setXY(0,0);
