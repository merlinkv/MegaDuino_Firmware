...
const char P_PRODUCT_NAME[] PROGMEM = "MegaDuino 2.5";
#include "Extras.h"
...
void loop(void) {
  ...
  if (millis() - timeDiff > 50) {   // check switch every 50ms 
    ...
    if(button_play()) {
      ...
      if(start==0) {
        ...
        //If a file is playing, pause or unpause the file                  
          ...
          #ifdef LCD16            
            if (currpct <100) {              
              utoa(newpct,PlayBytes,10);strcat_P(PlayBytes,PSTR("%"));lcd.setCursor(9,1);lcd.print(PlayBytes);
            }
            PlayBytes[0]=PlayBytes[1]=PlayBytes[2]='0';
            #ifndef cntrMSS
              if ((lcdsegs %1000) <10) utoa(lcdsegs%10,PlayBytes+2,10);
              else 
                 if ((lcdsegs %1000) <100)utoa(lcdsegs%1000,PlayBytes+1,10);
                 else 
                    utoa(lcdsegs%1000,PlayBytes,10);
            #endif
            #ifdef cntrMSS
              if ((lcdsegs %600) <10) utoa(lcdsegs%10,PlayBytes+2,10);
              else 
                 if ((lcdsegs %600) <60)utoa(lcdsegs%600,PlayBytes+1,10);
                 else 
                    utoa(lcdsegs%600 /60 *100 + lcdsegs%60,PlayBytes,10);
            #endif  
            lcd.setCursor(13,1);lcd.print(PlayBytes); 
          #elif defined(LCD20)
            if (currpct <100) {              
              utoa(newpct,PlayBytes,10);strcat_P(PlayBytes,PSTR("%"));lcd.setCursor(11,1);lcd.print(PlayBytes);
            }
            PlayBytes[0]=PlayBytes[1]=PlayBytes[2]='0';
            #ifndef cntrMSS
              if ((lcdsegs %1000) <10) utoa(lcdsegs%10,PlayBytes+2,10);
          ...
          #elif defined(OLED1306)
            if (currpct <100) {                         
              utoa(newpct,PlayBytes,10);strcat_P(PlayBytes,PSTR("%"));setXY (8,1);sendStr((unsigned char *)(PlayBytes));
    ...
    if(button_root() && start==0 && !lastbtn) {                                          // show min-max dir
          ...
          char len=0;
          lcd.setCursor(0,0); 
          lcd.print(utoa(oldMinFile,input,10)); lcd.print('<'); len += strlen(input) + 1;
          lcd.print(utoa(currentFile,input,10)); lcd.print('<'); len += strlen(input) + 1;
          lcd.print(utoa(oldMaxFile,input,10)); len += strlen(input); 
    ...
    if(start==1 && (oldMotorState!=motorState) && mselectMask) {  
      if(motorState==1 && !pauseOn){
        ...
        lcd.setCursor(0,1); lcd.print("-----PRESS PLAY-----");    
          ...
          //printtext(strcat_P(PlayBytes,PSTR(" bytes")),1);          
          setXY(0,1); sendStr((unsigned char *)"Paused          ");          
      ...
      if(motorState==0 && pauseOn) {
          ...
          if (lcdsegs % CNTRBASE != 0){utoa(lcdsegs%CNTRBASE,PlayBytes,10);lcd.setCursor(15,1);lcd.print(PlayBytes);} // // recálculo de decenas ... 10,20,30,40,50,60,70,80,90,110,120,..
          else 
          if (lcdsegs % (CNTRBASE*10) != 0) {utoa(lcdsegs%(CNTRBASE*10)/CNTRBASE*100,PlayBytes,10);lcd.setCursor(14,1);lcd.print(PlayBytes);} // recálculo de centenas ... 100,200,300,400,500,600,700,800,900,1100, ...
          lcdsegs++;                
        #elif defined(LCD20)
          String filename2 = fileName;
          lcd.setCursor(0,0); lcd.print("                    ");
          ...
          if (lcdsegs % CNTRBASE != 0){utoa(lcdsegs%CNTRBASE,PlayBytes,10);lcd.setCursor(18,1);lcd.print(PlayBytes);} // recálculo de decenas ... 10,20,30,40,50,60,70,80,90,110,120,..
          else 
          if (lcdsegs % (CNTRBASE*10) != 0) {utoa(lcdsegs%(CNTRBASE*10)/CNTRBASE*100,PlayBytes,10);lcd.setCursor(17,1);lcd.print(PlayBytes);} // recálculo de centenas ... 100,200,300,400,500,600,700,800,900,1100, ...
          lcdsegs++;
        #elif defined(OLED1306)
          #ifdef XY
            setXY(0,0);
...
void printtext(char* text, int l) {  //Print text to screen. 
  ...
  void OledStatusLine() {
      ...
      setXY(0,3); sendStr((unsigned char*)"ID:   BLK:  ");
        ...
        setXY(10,4);
        utoa(BAUDRATE,(char *)input,10);sendStr(input);
        setXY(10,5);
        if(mselectMask) sendStr((unsigned char *)"ON ");
        ...
        utoa(BAUDRATE,(char *)input,10);sendStr(input);
        setXY(5,7);
        if(mselectMask1) sendStr((unsigned char *)" M:ON");
        ...
        utoa(BAUDRATE,input,10);sendStrXY(input,10,4);
        if(mselectMask) sendStrXY("ON ",10,5);
...
void Banner() {
    ...
    sendStr((unsigned char *)"----------------");
    setXY(1,3);
    sendStr((unsigned char *)MVERSION);
    setXY(0,4);
    sendStr((unsigned char *)"----------------");
    delay(1500);
    reset_display();
  #elif defined(LCD16)
    lcd.setCursor(1,0); lcd.print(F(MVERSION));
    delay(1800);
    lcd.clear();
  #elif defined(LCD20)
    lcd.setCursor(0,0); lcd.print("--------------------");
    lcd.setCursor(3,1); lcd.print(F(MVERSION));
