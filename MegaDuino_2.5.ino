  ...
  if(button_down() && start==0
    ...
    if(start==1 && (oldMotorState!=motorState) && mselectMask==1 ) {  
      ...
      if(motorState==0 && pauseOn==1) {
          ...
          if (lcdsegs % CNTRBASE != 0){itoa(lcdsegs%CNTRBASE,PlayBytes,10);lcd.setCursor(14,1);lcd.print(PlayBytes);} // // recálculo de decenas ... 10,20,30,40,50,60,70,80,90,110,120,..
          else 
          if (lcdsegs % (CNTRBASE*10) != 0) {itoa(lcdsegs%(CNTRBASE*10)/CNTRBASE*100,PlayBytes,10);lcd.setCursor(13,1);lcd.print(PlayBytes);} // recálculo de centenas ... 100,200,300,400,500,600,700,800,900,1100, ...
          lcdsegs++;                
        #elif defined(LCD20)
          String filename2 = fileName;
          lcd.setCursor(0,0); lcd.print("                    ");
