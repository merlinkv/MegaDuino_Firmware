...
unsigned long filesize;             // filesize used for dimensioning AY files
  ...
  #define btnUp         PA0            // UP Button
  #define btnDown       PA1            // Down Button
  #define btnPlay       PA2            // Play button
  #define btnStop       PA3            // Stop button
  #define btnRoot       PA4            // Root - Return to SD card root
  #define btnDelete     PA5            // Del Two options: Delete files or reset the board
...
void SetPlayBlock(){
  printtextF(PSTR(" "),0);
    ...
    if (bytesRead > 0){
      lcd.print(F("ID:"));lcd.print(currentID,HEX); // Block ID en hex
      ...
      #else
        setXY(0,3); sendStr((unsigned char *)"ID:");
        setXY(3,3);
        if (currentID/16 < 10) input[0]=48+currentID/16;
          else input[0]=55+currentID/16;
        if (currentID%16 < 10) input[1]=48+currentID%16;
        ...
        setXY(6,3); sendStr((unsigned char *)"BLK:");
        if (block < 10 ) {
          setXY(10,3); sendStr("0");
          setXY(11,3); utoa(block, (char *)input, 11);sendStr(input);//sendChar(' ');  
        } else {
          setXY(10,3); utoa(block, (char *)input, 10);sendStr(input);//sendChar(' ');
        }
      #endif
    #endif      
    currpct=100; 
